...
class Camera:
    ...
    def calibrate_camera(self, pictures, save_data_in_npz: bool):
        ...
        img0 = cv.cvtColor(cv.imread(f"../data/images/calibration/{pictures[0]}.png"), cv.COLOR_BGR2GRAY)

        img_counter = 0

        for f_name in pictures:
            # print(f_name)
            img = cv.imread(f"../data/images/calibration/{f_name}.png")
            ...
            if ret:
                ...
                cv.imwrite(f"../data/images/calibration/chessboard_corners/chessboard_corner_{img_counter}.png", img)
                img_counter += 1
